...
describe('RegistrationController', () => {
  ...
  beforeEach(async () => {
    const moduleRef: TestingModule = await Test.createTestingModule({
      ...
      providers: [GameService, EventEmitter2, RoomRepositoryService, PlayerRepositoryService, LeaderboardsRepositoryService, {
  ...
  it('should equal to false response dto', () => {
    controller.validate(request)
      .then(data => expect(data).toStrictEqual({lobby: response.lobby, name: response.name}))
      .catch();
  });
